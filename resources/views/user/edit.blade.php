<x-app-layout>
    ...
    <div class="py-12">
        <form method="POST" action="{{ route('user.update', $user->id) }}" class="max-w-xl mx-auto">
            ...
            <x-text-input id="name" name="name" value="{{$user->name}}" type="text" required class="mt-1 block w-full" />

            <x-input-label for="email" :value="__('Email')" class="mt-4" />
            <x-text-input id="email" name="email" value="{{$user->email}}" type="email" required class="mt-1 block w-full" />
